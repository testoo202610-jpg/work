...
export interface Projectile { id: string; attackerId: string; attackerType: UnitType; faction: Faction; targetId: string; x: number; y: number; speed: number }
...
export function buildPath(grid: PathGrid, start: GridPoint, goal: GridPoint): GridPoint[] { const key = (point: GridPoint) => `${point.col},${point.row}`; const inside = (point: GridPoint) => point.col >= 0 && point.row >= 0 && point.col < grid.width && point.row < grid.height; const isBlocked = (point: GridPoint) => grid.blocked[point.row]?.[point.col] ?? true; if (!inside(start) || !inside(goal) || isBlocked(start) || isBlocked(goal)) return []; const open: GridPoint[] = [start]; const came = new Map<string, GridPoint>(); const cost = new Map<string, number>([[key(start), 0]]); const heuristic = (point: GridPoint) => Math.hypot(point.col - goal.col, point.row - goal.row); while (open.length) { open.sort((a, b) => (cost.get(key(a)) ?? Infinity) + heuristic(a) - ((cost.get(key(b)) ?? Infinity) + heuristic(b))); const current = open.shift()!; if (current.col === goal.col && current.row === goal.row) { const path: GridPoint[] = [current]; let cursor = current; while (came.has(key(cursor))) { cursor = came.get(key(cursor))!; path.unshift(cursor) } return path } for (const delta of [{ col: 1, row: 0 }, { col: -1, row: 0 }, { col: 0, row: 1 }, { col: 0, row: -1 }, { col: 1, row: 1 }, { col: -1, row: -1 }, { col: 1, row: -1 }, { col: -1, row: 1 }]) { const next = { col: current.col + delta.col, row: current.row + delta.row }; if (!inside(next) || isBlocked(next)) continue; if (delta.col !== 0 && delta.row !== 0 && (isBlocked({ col: current.col + delta.col, row: current.row }) || isBlocked({ col: current.col, row: current.row + delta.row }))) continue; const nextKey = key(next); const nextCost = (cost.get(key(current)) ?? Infinity) + (delta.col !== 0 && delta.row !== 0 ? 1.414 : 1); if (nextCost < (cost.get(nextKey) ?? Infinity)) { came.set(nextKey, current); cost.set(nextKey, nextCost); if (!open.some((point) => point.col === next.col && point.row === next.row)) open.push(next) } } } return [] }
export function createProjectile(attacker: Unit, target: Unit | Building, id: string): Projectile { return { id, attackerId: attacker.id, attackerType: attacker.type, faction: attacker.faction, targetId: target.id, x: attacker.x, y: attacker.y, speed: 420 } }
export function advanceProjectile(projectile: Projectile, target: Point, dt: number): Projectile { const distance = Math.hypot(target.x - projectile.x, target.y - projectile.y); if (distance === 0) return projectile; const step = projectile.speed * dt; if (distance <= step) return { ...projectile, x: target.x, y: target.y }; return { ...projectile, x: projectile.x + ((target.x - projectile.x) / distance) * step, y: projectile.y + ((target.y - projectile.y) / distance) * step } }
export function projectileReached(projectile: Projectile, target: Point): boolean { return Math.hypot(target.x - projectile.x, target.y - projectile.y) < 1 }
